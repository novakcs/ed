	...
	void drawPiece(Canvas canvas, Chess.ChessPiece piece, double minX, double minY, double width, double height) {
		...
		if (piece == movingPiece){
			gc.setFill(Color.GREY);
                        ...
                        /*
                        // Voy a implementar aquí lo de pintar los cuadraditos de rojo
                        
                        
                        Bounds bounds = canvas.getBoundsInLocal();
                        Bounds boardBounds = getBoardBounds(canvas);
                        
                        // Tengo que obtener las posiciones que puedo mover y almacen y si es esa, pinto de rojo
                        PiecePosition[] posiciones;
                        posiciones = piece.getAvailablePositions(board);
                        for (PiecePosition posicion : posiciones) {
                             gc.setFill(Color.RED);
                             gc.fillRect(boardBounds.getMinX() + posicion.getColumn() * width, boardBounds.getMinY() + posicion.getRow() * height, 
                             boardBounds.getMinX() + (posicion.getColumn() + 1) * width, boardBounds.getMinY() + (posicion.getRow() + 1) * height);
                        }
                        */
                }
		...
		switch(piece.getType()) {
			...
			case KNIGHT:                                
                                gc.fillRect(minX, minY + height * 0.7, width, height * 0.3);
                                drawTriangle(gc, minX, minY + height * 0.2, width, height * 0.8);
		...
		for (int i = 0; i < 8; i++) {
			for (int j = 0; j < 8; j++) {
					...
					gc.setFill(Color.YELLOW);
                                // Ahora si hay una ficha seleccionada, PINTO DE ROJO LOS CUADRADOS POSIBLES
                                if (getMovingPiece() != null) {                               
                                    // Tengo que obtener las posiciones que puedo mover y almacen y si es esa, pinto de rojo
                                    PiecePosition[] posiciones = getMovingPiece().getAvailablePositions(board);
                                    
                                    for (PiecePosition posicion : posiciones) {
                                        if (posicion.getColumn() == i && posicion.getRow() == j) {
                                            gc.setFill(Color.RED);
                                           break;
                                        }
                                    }
